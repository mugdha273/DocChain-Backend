...
class DocumentViews(viewsets.ModelViewSet):
    ...
    @action(methods=['get'], detail=False,url_path='bool-docs', permission_classes = [permissions.IsAuthenticated])
    def booldocs(self, request, format=None):
        doc = DocumentModel.objects.filter(user=request.user)
        serializer = DocumentSerializer(doc, many=True).data
        data= dict()
        data = [*serializer]
